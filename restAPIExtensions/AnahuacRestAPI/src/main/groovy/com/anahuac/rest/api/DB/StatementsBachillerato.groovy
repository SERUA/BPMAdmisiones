...
class StatementsBachillerato {
	...
	public static final String GET_DESCUENTOS_CIUDAD_BACHILLERATO="SELECT c.* FROM CatDescuentos c WHERE c.isEliminado = false AND CURRENT_TIMESTAMP BETWEEN  TO_DATE(c.inicioVigencia,'YYYY-MM-DD\"T\"HH24:MI:SS') AND TO_DATE(c.finVigencia,'YYYY-MM-DD\"T\"HH24:MI:SS') AND c.campus = ? AND (c.bachillerato = ? OR c.ciudad = ?) ORDER BY c.descuento DESC";
	public static final String GET_DESCUENTOS_CAMPANA="SELECT c.* FROM CatDescuentos c WHERE c.tipo = 'Campaña' AND isEliminado is false AND CURRENT_TIMESTAMP BETWEEN  TO_DATE(c.inicioVigencia,'YYYY-MM-DD\"T\"HH24:MI:SS') AND TO_DATE(c.finVigencia,'YYYY-MM-DD\"T\"HH24:MI:SS') AND c.campus = ? ORDER BY c.persistenceId ASC"
