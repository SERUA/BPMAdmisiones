	...
	RestApiResponse doHandle(HttpServletRequest request, RestApiResponseBuilder responseBuilder, RestAPIContext context) {
		...
		try{
			switch(url) {
				...
				case "getCarreraByCampus":
				try{
					String campus = request.getParameter "campus";
					result = new NuevoListadoDAO().getCarreraByCampus(campus, context);
					if (result.isSuccess()) {
						return buildResponse(responseBuilder, HttpServletResponse.SC_OK, new JsonBuilder(result).toString())
					}else {
						return buildResponse(responseBuilder, HttpServletResponse.SC_INTERNAL_SERVER_ERROR,  new JsonBuilder(result).toString())
				...
				case "getPeriodo":
				result = new NuevoListadoDAO().getPeriodo()
				if (result.isSuccess()) {
					return buildResponse(responseBuilder, HttpServletResponse.SC_OK, new JsonBuilder(result.getData()).toString())
				}else {
					return buildResponse(responseBuilder, HttpServletResponse.SC_INTERNAL_SERVER_ERROR,  new JsonBuilder(result).toString())
...
//					result = new ConektaDAO().getBitacoraPagosByEmail(email, Long.valueOf(caseId), context);
				...
				case "getB64FileByUrlAzure":
					...
					result = new SolicitudUsuarioDAO().getB64FileByUrlAzure(urlAzure);
					responseBuilder.withMediaType("application/json");
					if (result.isSuccess()) {
						return buildResponse(responseBuilder, HttpServletResponse.SC_OK, new JsonBuilder(result.getData()).toString())
					}else {
				...
				case "getUsuarioExiste":
					String usuario=request.getParameter "usuario"
					result = new UsuariosDAO().getUsuarioExiste(usuario);
					responseBuilder.withMediaType("application/json")
					if (result.isSuccess()) {
						return buildResponse(responseBuilder, HttpServletResponse.SC_OK, new JsonBuilder(result).toString())
					}else {
