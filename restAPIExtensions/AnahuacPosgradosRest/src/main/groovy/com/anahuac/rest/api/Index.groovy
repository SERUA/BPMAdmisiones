...
//					break;
				...
				break;
				case "updateSesion":
					result = new SesionesDAO().updateSesion(jsonData)
					if (result.isSuccess()) {
						return buildResponse(responseBuilder, HttpServletResponse.SC_OK, new JsonBuilder(result).toString())
					}else {
						return buildResponse(responseBuilder, HttpServletResponse.SC_INTERNAL_SERVER_ERROR,  new JsonBuilder(result).toString())
...
//					}
				...
				case "selectSolicitudesAdmision":
					...
					break;
				case "updateEstatusSolicitudByCaseid":
					String estatus = request.getParameter "estatus"
					String caseid = request.getParameter "caseid"
					result = new SolicitudDeAdmisionDAO().updateEstatusSolicitudByCaseid(Long.valueOf(caseid), estatus);
					if (result.isSuccess()) {
						return buildResponse(responseBuilder, HttpServletResponse.SC_OK, new JsonBuilder(result).toString())
					}else {
						return buildResponse(responseBuilder, HttpServletResponse.SC_INTERNAL_SERVER_ERROR,  new JsonBuilder(result).toString())
				...
				case "selectBitacoraTransferencias":
					result = new SolicitudDeAdmisionDAO().selectBitacoraTransferencias(jsonData, context);
					if (result.isSuccess()) {
						return buildResponse(responseBuilder, HttpServletResponse.SC_OK, new JsonBuilder(result).toString())
					}else {
						return buildResponse(responseBuilder, HttpServletResponse.SC_INTERNAL_SERVER_ERROR,  new JsonBuilder(result).toString())
					...
					break;
				case "updateAgendadoHorario":
					result = new SesionesDAO().updateAgendadoHorario(jsonData, context);
					if (result.isSuccess()) {
						return buildResponse(responseBuilder, HttpServletResponse.SC_OK, new JsonBuilder(result).toString())
					}else {
						return buildResponse(responseBuilder, HttpServletResponse.SC_INTERNAL_SERVER_ERROR,  new JsonBuilder(result).toString())
					...
					break;
				case "agendarHorario":
					result = new SesionesDAO().agendarHorario(jsonData, context);
					if (result.isSuccess()) {
						return buildResponse(responseBuilder, HttpServletResponse.SC_OK, new JsonBuilder(result).toString())
					}else {
						return buildResponse(responseBuilder, HttpServletResponse.SC_INTERNAL_SERVER_ERROR,  new JsonBuilder(result).toString())
				...
				case "createOrUpdatePosgrado":
					...
					break;
				case "getExcelFileGeneralProceso":
					result = new DocumentosTextosDAO().getExcelFileGeneralProceso(jsonData, context);
					if (result.isSuccess()) {
						return buildResponse(responseBuilder, HttpServletResponse.SC_OK, new JsonBuilder(result).toString())
					}else {
						return buildResponse(responseBuilder, HttpServletResponse.SC_INTERNAL_SERVER_ERROR,  new JsonBuilder(result).toString())
