...
class StatementsCatalogos {
	...
	//PSGRCatCampus
	public static final String INSERT_CATCAMPUS = "INSERT INTO PSGRCatCampus (persistenceid, calle, clave, codigo_postal, colonia, descripcion, email, fecha_creacion, fecha_implementacion, grupo_bonita, municipio, numero_ext, numero_int, orden, url_aviso_privacidad, url_imagen, usuario_banner, estados_relacion_pid, paises_relacion_pid, id, activado, eliminado, tiene_posgrado)  VALUES (COALESCE((SELECT MAX(persistenceid)::TEXT FROM PSGRCATcampus), '0')::INTEGER + 1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
	public static final String DELETE_CATCAMPUS  = "UPDATE PSGRCatCampus SET eliminado = ? WHERE persistenceid = ?";
	public static final String UPDATE_CATCAMPUS  = "UPDATE PSGRCatCampus SET calle=?, clave=?, codigo_postal=?, colonia=?, descripcion=?, email=?, fecha_creacion=?, fecha_implementacion=?, grupo_bonita=?, municipio=?, numero_ext=?, numero_int=?, orden=?, url_aviso_privacidad=?, url_imagen=?, usuario_banner=?, estados_relacion_pid = ?, paises_relacion_pid = ?, id=?, tiene_posgrado=? WHERE persistenceid=?;";
...
//	public static final String GET_CATGESTIONESCOLAR = "SELECT GE.*, campus.descripcion as nombreCampus FROM PSGRCATGESTIONESCOLAR as GE  LEFT JOIN psgrcatcampus campus ON campus.grupo_bonita = GE.campus [CAMPUS]  [WHERE] [ORDERBY] [LIMITOFFSET]";
	...
	public static final String GET_CATGESTIONESCOLAR_BY_CAMPUS_MULTIPLE = "SELECT GE.*, posgrado.descripcion AS posgrado, periodo.descripcion AS periodo, campus.descripcion as nombreCampus FROM psgrcatgestionescolar as GE LEFT JOIN psgrcatcampus campus ON campus.descripcion = GE.campus LEFT JOIN PSGRCatPosgrado posgrado ON posgrado.persistenceid = GE.posgrado_pid LEFT JOIN PSGRCatPeriodo periodo ON periodo.persistenceid = GE.periodo_pid [WHERE]";
	...
	public static final String SELECT_CATPOSGRADO_DESCRIPCION_DISTINC  = "SELECT DISTINCT descripcion FROM PSGRCatPosgrado [WHERE];";
	...
	public static final String GET_GESTIONESCOLARBYIDCAMPUS  = "SELECT * FROM PSGRCatGestionEscolar WHERE campus_referencia_pid = ? AND is_eliminado <> true AND posgrado_pid IS NOT NULL ORDER BY posgrado_pid";
