...
class MenuParent extends Menu{
	public static String GET="SELECT DISTINCT * FROM (SELECT bam.id, bam.parentid, CASE WHEN parent.displayname IS NULL THEN true ELSE false END                                     isparent, '../apps/'||ba.token||'/'||bap.token AS url, ba.token                                parenttoken, bam.displayname                         menu, bam.index_, bap.token, ba.displayname, parent.displayname parent FROM business_app ba LEFT JOIN public.business_app_menu bam ON bam.applicationid=ba.id LEFT JOIN public.business_app_page bap ON bap.id=bam.applicationpageid LEFT JOIN public.business_app_menu parent ON bam.parentid=parent.id INNER JOIN app_menu_role apr ON apr.business_app_menuid=bam.displayname WHERE ba.displayname='Administrativo posgrados' AND (apr.roleid IN (SELECT um.roleid FROM user_membership um WHERE userid=?)) ORDER BY parent.displayname DESC, bam.index_ ) datos ORDER BY isparent desc, parent DESC, index_ ASC;"
