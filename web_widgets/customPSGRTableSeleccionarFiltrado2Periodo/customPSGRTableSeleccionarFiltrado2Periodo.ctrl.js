function PbTableCtrl($scope, $http, $location, $log, $window, localStorageService, modalService,blockUI) {
    ...
    function doRequestDelete(method, url, params) {
        ...
        //datos.lstCatPeriodoInput[0].campus.persistenceId_string = datos.lstCatPeriodoInput[0].campus.persistenceId;
        let inicio = typeof datos.lstCatPeriodoInput[0].fechaInicio === 'string' ? (new Date(angular.copy(datos.lstCatPeriodoInput[0].fechaInicio.slice(0,10)) + "Z")):angular.copy(datos.lstCatPeriodoInput[0].fechaInicio) ;
        let fin = typeof datos.lstCatPeriodoInput[0].fechaFin === 'string' ? (new Date(angular.copy(datos.lstCatPeriodoInput[0].fechaFin.slice(0,10)) + "Z")):angular.copy(datos.lstCatPeriodoInput[0].fechaFin) ;
        datos.lstCatPeriodoInput[0].fechaInicio = inicio;
         datos.lstCatPeriodoInput[0].fechaFin = fin;
        var req = {
            method: method,
            url: url,
            data: datos,
