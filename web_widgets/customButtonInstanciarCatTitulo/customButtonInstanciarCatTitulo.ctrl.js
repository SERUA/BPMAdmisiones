...
function startProcess() {
    debugger
        if ($scope.properties.dataToChange2.clave || $scope.properties.dataToChange2.clave === "") {
            ...
            if ($scope.properties.dataToChange2.lstCatTituloInput[0].clave && $scope.properties.dataToChange2.lstCatTituloInput[0].descripcion && $scope.properties.dataToChange2.lstCatTituloInput[0].orden) {
                if ($scope.properties.processId) {
                    var prom = doRequest('POST', '../API/bpm/process/' + $scope.properties.processId + '/instantiation', $scope.properties.userId).then(function () {
                        doRequest("GET", $scope.properties.url).then(function () {
                            $scope.properties.dataToChange = $scope.properties.dataToSet;
                ...
                if (!$scope.properties.dataToChange2.lstCatTituloInput[0].descripcion) {
                    swal("¡Falto capurar informacion en!", "Descripcion" , "warning");
                }
                if (!$scope.properties.dataToChange2.lstCatTituloInput[0].clave) {
                    swal("¡Falto capurar informacion en!", "Clave" , "warning");
                }
                if (!$scope.properties.dataToChange2.lstCatTituloInput[0].orden) {
                    swal("¡Falto capurar informacion en!", "Orden" , "warning");
                }
            }
        }
