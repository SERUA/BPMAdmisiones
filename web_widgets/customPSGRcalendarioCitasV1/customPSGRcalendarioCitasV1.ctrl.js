    function($scope, $http, blockUI, $window) {
        ...
        $scope.citaValorAnterior = undefined;
        
        $scope.$watch("properties.cita", () => {
            if ($scope.properties.cita && $scope.citaValorAnterior === undefined) {
                $scope.citaValorAnterior = angular.copy($scope.properties.cita);
            }
        });
        ...
        $scope.seleccionarHorario = function (_horario){
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    // Actualizando el nuevo valor anterior
                    $scope.citaValorAnterior = angular.copy($scope.properties.cita);
            ...
            for (let evento of eventos) {
                if (evento.id === parseInt(_id)) {
                    ...
                    $http.get(url).success(function(_data){
                        try {
                            $scope.fechaSelected = fechaAStringConFormato(evento.start_date);
                        }catch(e) {
                            $scope.fechaSelected = "";
                        }
