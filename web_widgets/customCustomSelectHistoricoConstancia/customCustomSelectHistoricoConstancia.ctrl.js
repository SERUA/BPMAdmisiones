function PbSelectCtrl($scope, $parse, $log, widgetNameFactory, $timeout, $window, $element) {
    ...
    function getListaConstancia(){
        $scope.properties.availableValues = [];
        for(let i = 0; i < $scope.properties.ListaConstanciaFile.length ; i++){
            let newFile = $scope.properties.ListaConstanciaFile[i];
            let urlParts = newFile.urlConstancia.split("/");
            newFile.fileName = (urlParts[urlParts.length -1]).replace("%20"," ");
    ...
    $scope.$watch("properties.ListaConstanciaFile", function(){
        if($scope.properties.ListaConstanciaFile !== undefined){
            initWatcher();
        }
    });

    function initWatcher(){
        $scope.$watch("properties.azureTempKeyRes", function(){
            if($scope.properties.azureTempKeyRes !== undefined){
                getListaConstancia();
            }
        });
    }
    ...
    $scope.$watch("properties.context", function(){
        if($scope.properties.context !== undefined){
            $scope.properties.index = $scope.properties.context.constancia_ref.length - 1;
            $scope.properties.listaDocumentoBdm = $scope.properties.context.constancia_ref;
        }
    });
}