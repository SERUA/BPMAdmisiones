function PbDatePickerCtrl($scope, $log, widgetNameFactory, $element, $locale, $bsDatepicker) {
    ...
    $scope.$watch("properties.value", function(){
        var fechaminima = new Date(new Date().getTime() - 410240000000);
        console.log($scope.properties.value);
        if($scope.properties.value.getTime() > fechaminima.getTime()){
            console.log("mayor");
            swal("¡Fecha de nacimiento!","La fecha de nacimiento ingresada no cumple con los requisitos necesarios de edad, dado que indica que eres menor a 13 años.","warning");
