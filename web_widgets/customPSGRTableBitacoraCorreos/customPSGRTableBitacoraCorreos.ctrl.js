    ...
    function doRequestGet() {
        ...
        for (let index = 0; index < dataToSend.lstFiltro.length; index++) {
            ...
            if (element.columna == "CAMPUS") {
                dataToSend.lstFiltro[index].valor = $scope.properties.campusSelected.grupo_bonita;
                notfound = false;
            }
        }
        if (notfound) {
        ...
        var req = {
            ...
            headers: {
                'Content-Type': 'application/json'  // Asegúrate de establecer el encabezado adecuado
            },
            data: dataToSend
        };
