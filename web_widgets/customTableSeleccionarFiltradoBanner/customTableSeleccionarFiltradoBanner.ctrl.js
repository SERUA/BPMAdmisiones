function PbTableCtrl($scope, $http, $location, $log, $window, localStorageService, modalService) {
    ...
    this.selectRowEnable = function(row)  {
        console.log(row)
        swal("¿Esta seguro que desea "+ (row["isEnabled"] ? "desactivar" : "activar") +" la nacionalidad?", {
                buttons: {
                    cancel: "No",
                    catch: {
                        text: "Si",
    ...
    function startProcess2() {
        ...
        if ($scope.properties.processId) {
            var prom = doRequestEnable('POST', '../API/bpm/process/' + $scope.properties.processId + '/instantiation', $scope.properties.userId).then(function() {
                localStorageService.delete($window.location.href);
            });
    ...
    function doRequestEnable(method, url, params) {
        debugger;
        var req = {
            method: method,
            url: url,
        ...
        return $http(req)
            .success(function(data, status) {
                ...
                swal("!Nacionalidad "+($scope.properties.dataToSend.lstCatNacionalidadInput[0].isEnabled ? "activado" : "desactivado")+" correctamente!", "", "success");
