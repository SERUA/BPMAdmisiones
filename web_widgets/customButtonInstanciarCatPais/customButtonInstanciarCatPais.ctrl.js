function PbButtonCtrl($scope, $http, $location, $log, $window, localStorageService, modalService) {
   ...
   function startProcess() {
	...
	if ($scope.properties.dataToChange2.clave || $scope.properties.dataToChange2.clave === "") {
		if ($scope.properties.dataToChange2.orden && $scope.properties.dataToChange2.clave && $scope.properties.dataToChange2.descripcion) {
		...
		if ($scope.properties.dataToChange2.lstCatPaisInput[0].orden && $scope.properties.dataToChange2.lstCatPaisInput[0].clave && $scope.properties.dataToChange2.lstCatPaisInput[0].descripcion) {
			if ($scope.properties.processId) {
				var prom = doRequest('POST', '../API/bpm/process/' + $scope.properties.processId + '/instantiation', $scope.properties.userId).then(function () {
					doRequest("GET", $scope.properties.url).then(function () {
						$scope.properties.dataToChange = $scope.properties.dataToSet;
			...
			if (!$scope.properties.dataToChange2.lstCatPaisInput[0].descripcion) {
				swal("¡Falto capurar informacion en!", "Descripción", "warning");
			}
			if (!$scope.properties.dataToChange2.lstCatPaisInput[0].clave) {
				swal("¡Falto capurar informacion en!", "Clave", "warning");
			}
			if (!$scope.properties.dataToChange2.lstCatPaisInput[0].orden) {
				swal("¡Falto capurar informacion en!", "Orden", "warning");
			}
		}
	}
