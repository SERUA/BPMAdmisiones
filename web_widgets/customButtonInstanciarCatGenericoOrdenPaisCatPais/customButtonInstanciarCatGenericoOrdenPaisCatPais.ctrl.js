function PbButtonCtrl($scope, $http, $location, $log, $window, localStorageService, modalService) {
    ...
    function doRequest(method, url, params) {
        ...
        return $http(req)
            .success(function(data, status) {
                ...
                if ($scope.properties.targetUrlOnSuccess && method !== 'POST') {
                    redirectIfNeeded();
                }
                closeModal($scope.properties.closeOnSuccess);
            })
    ...
    function checkorder(funcion) {
        ...
        if (funcion === 'agregar') {
            var req = {
                method: 'POST',
                url: "/bonita/API/extension/posgradosRest?url=getValidarOrden&p=0&c=10&tabla=CATPAIS&orden=" + $scope.properties.dataToChange2[$scope.properties.nombreTabla][0].orden + "&id="
            };
        } else {
            var req = {
                method: 'POST',
                url: "/bonita/API/extension/posgradosRest?url=getValidarOrden&p=0&c=10&tabla=CATPAIS&orden=" + $scope.properties.dataToChange2.orden + "&id=" + $scope.properties.dataToChange2.persistenceId
            };
        }
    ...
    function checkclave(funcion) {
        ...
        if (funcion === 'agregar') {
            var req = {
                method: 'GET',
                url: "/bonita/API/extension/posgradosRest?url=getValidarClave&p=0&c=10&tabla=CATPAIS&clave=" + $scope.properties.dataToChange2[$scope.properties.nombreTabla][0].clave + "&id="
            };
        } else {
            var req = {
                method: 'GET',
                url: "/bonita/API/extension/posgradosRest?url=getValidarClave&p=0&c=10&tabla=CATPAIS&clave=" + $scope.properties.dataToChange2.clave + "&id=" + $scope.properties.dataToChange2.persistenceId
            };
        }
